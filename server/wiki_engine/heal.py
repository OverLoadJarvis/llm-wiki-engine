...
class HealWorkflow:
    ...
    def heal_missing_entities(
        ...
        missing_entities = self._find_missing_entities(kb_id, pages, min_refs)
        ...
        for entity in missing_entities:
            ...
            try:
                ...
                self.db.add_file(kb_id, entity_path, content)
    ...
    def _find_missing_entities(
        self, kb_id: int, pages: list[dict], min_refs: int = 3
    ) -> list[str]:
    ...
    def _generate_entity_page(
        self, kb_id: int, entity: str, sources: list[dict], model: str
    ) -> str:
        """调用 LLM 生成实体定义页面。
