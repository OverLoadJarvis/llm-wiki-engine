...
def main() -> None:
    ...
    p_build.add_argument("--project", required=True, help="项目名称")
    ...
    p_update.add_argument("--project-id", type=int, required=True)
    ...
    p_query.add_argument("--project-id", type=int, required=True)
    ...
    p_lint.add_argument("--project-id", type=int, required=True)
    ...
    p_health.add_argument("--project-id", type=int, required=True)

    # graph
    p_graph = sub.add_parser("graph", help="构建知识图谱")
    p_graph.add_argument("--db", default="storage/wiki.db")
    p_graph.add_argument("--project-id", type=int, required=True)
    ...
    # stats
    p_stats = sub.add_parser("stats", help="项目统计")
    p_stats.add_argument("--db", default="storage/wiki.db")
    p_stats.add_argument("--project-id", type=int, required=True)

    # heal
    p_heal = sub.add_parser("heal", help="图谱自愈")
    p_heal.add_argument("--db", default="storage/wiki.db")
    p_heal.add_argument("--project-id", type=int, required=True)
    ...
    try:
        if args.command == "build":
            result = engine.quick_build(args.project, args.source)
