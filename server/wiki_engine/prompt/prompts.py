...
你正在为知识库摄入选择来源页格式模板。根据文件名与文档片段，从下列可选格式中选出最合适的 id。
...
规则:
- 只从可选格式中选择 id（3=通用来源页；4a=日记；4b=会议；4c=Excel/表格；4d=博客）
- 若明显属于某一领域模板，返回对应 4x；可多选仅当文档同时明显跨多种领域
- 无法判断或不属于任何领域时，返回 ["3"]
- 不要编造不存在的 id

只返回一个有效的 JSON 对象(不要 markdown 代码围栏，不要 JSON 之外的任何文字):
{{"format_ids": ["3"], "reason": "一句话说明选择理由"}}
"""
